                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _StatusCard(
                            title: 'MQTT Broker',
                            status: controller.brokerConnected ? 'Connected' : 'Disconnected',
                            color: controller.brokerConnected ? Colors.green : Colors.red,
                            icon: controller.brokerConnected ? Icons.wifi : Icons.wifi_off,
                        ...
                        Expanded(
                          child: _StatusCard(
                            title: 'ESP32 Device',
                            status: controller.deviceOnline ? 'Online' : 'Offline',
                            color: controller.deviceOnline ? Colors.blue : Colors.grey,
                            icon: controller.deviceOnline ? Icons.developer_board : Icons.developer_board_off,
                      ...
                      child: Column(
                        children: [
                          _ControlCard(
                            ...
                            onChanged: controller.brokerConnected && controller.deviceOnline
                                ? (value) => controller.toggleDevice('light')
                                : null,
                            subtitle: 'Status: ${controller.lightState.toUpperCase()}',
                          ...
                          _ControlCard(
                            ...
                            onChanged: controller.brokerConnected && controller.deviceOnline
                                ...
                                : null,
                            subtitle: 'Status: ${controller.fanState.toUpperCase()}',
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 16),
                                    _InfoRow('📡 WiFi Signal', '${controller.rssi} dBm'),
                                    _InfoRow('💿 Firmware', controller.firmware),
                                    _InfoRow('⏰ Last Update', controller.lastUpdate),
                                  ],
                                ),
                              ),
                            ),
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: controller.isConnecting ? null : controller.connect,
                            style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.transparent,
                            ...
                            label: Text(
                              controller.isConnecting ? 'Connecting...' : 'Reconnect to Broker',
...
// _StatusCard and _ControlCard remain unchanged
...
// MqttController with proper null-safety
class MqttController extends ChangeNotifier {
  ...
  void _handleMessage(String topic, String payload) {
    try {
      ...
      if (topic == _deviceStateTopic) _handleDeviceState(data);
      else if (topic == _sysOnlineTopic) _handleOnlineStatus(data);
  ...
  void _handleDeviceState(Map<String, dynamic> data) {
    if (data.containsKey('light')) _lightState = data['light'] as String;
    if (data.containsKey('fan')) _fanState = data['fan'] as String;
    if (data.containsKey('rssi')) _rssi = data['rssi'].toString();
    if (data.containsKey('fw')) _firmware = data['fw'] as String;
  }

  void _handleOnlineStatus(Map<String, dynamic> data) {
    if (data.containsKey('online')) _deviceOnline = data['online'] as bool;
  }
  ...
  void toggleDevice(String device) {
    ...
    if (device == 'light') {
      newState = (_lightState == 'on') ? 'off' : 'on';
      _lightState = newState;
    } else if (device == 'fan') {
      newState = (_fanState == 'on') ? 'off' : 'on';
      _fanState = newState;
    } else {
      return;
    }

    final command = jsonEncode({device: newState});
